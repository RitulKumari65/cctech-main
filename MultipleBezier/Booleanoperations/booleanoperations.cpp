...
booleanoperations::booleanoperations(QWidget *parent)
    ...
    connect(applyBooleanButton, &QPushButton::clicked, this, &booleanoperations::onApplyBooleanOperation);
...
void booleanoperations::initializeGL() {
    initializeOpenGLFunctions();
    glClearColor(0.1f, 0.1f, 0.1f, 1.0f);
    glEnable(GL_DEPTH_TEST);
}

void booleanoperations::resizeGL(int w, int h) {
    glViewport(0, 0, w, h);
}

void booleanoperations::paintGL() {
    ...
    for (const auto &polygon : polygons) {
        ...
        if (polygon.isClosed) {
            ...
            for (int i = 0; i < polygon.points.size(); ++i) {
                ...
                QVector3D top1 = base1 + polygon.normal * BooleanHeight;
                QVector3D top2 = base2 + polygon.normal * BooleanHeight;
...
void booleanoperations::mousePressEvent(QMouseEvent *event) {
...
void booleanoperations::mouseMoveEvent(QMouseEvent *event) {
    if (rightMousePressed) {
        ...
        BooleanHeight = dy * 0.01f;
        qDebug() << "Height:" << BooleanHeight;
        update();
    }
}

void booleanoperations::mouseReleaseEvent(QMouseEvent *event) {
    if (event->button() == Qt::RightButton) {
        rightMousePressed = false;
    }
}

QVector3D booleanoperations::screenToWorld(const QPoint &pos) {
    float x = (2.0f * pos.x()) / width() - 1.0f;
    float y = 1.0f - (2.0f * pos.y()) / height();
    return QVector3D(x, y, 0.0f);
}

bool booleanoperations::isCloseToFirstPoint(const QVector3D &p, const Polygon &polygon) {
    if (polygon.points.isEmpty()) return false;
    return (polygon.points.first() - p).length() < 0.05f;
}

QVector3D booleanoperations::computePolygonNormal(const QVector<QVector3D> &points) {
...
bool booleanoperations::pointInPolygon(const QPointF& point, const std::vector<QPointF>& polygon) {
    int count = 0;
...
bool booleanoperations::segmentsIntersect(const QPointF& a1, const QPointF& a2, const QPointF& b1, const QPointF& b2, QPointF& out) {
...
void booleanoperations::addAndSortIntersections(std::vector<QPointF>& result, const QPointF& point) {
    if (std::find(result.begin(), result.end(), point) == result.end()) {
        result.push_back(point);
    }
}

std::vector<QPointF> booleanoperations::splitEdges(const std::vector<QPointF>& polygon, const std::vector<QPointF>& intersections) {
...
std::vector<QPointF> booleanoperations::intersect(const std::vector<QPointF>& poly1, const std::vector<QPointF>& poly2) {
...
std::vector<QPointF> booleanoperations::unionPolygons(const std::vector<QPointF>& poly1, const std::vector<QPointF>& poly2) {
vector<QPointF> intersections;
...
std::vector<QPointF> booleanoperations::subtractPolygons(const std::vector<QPointF>& poly1, const std::vector<QPointF>& poly2) {
...
void booleanoperations::onApplyBooleanOperation() {
    if (polygons.size() < 2) {
...
QPainterPath booleanoperations::convertToPath(const Polygon &shape) {
    QPainterPath path;
